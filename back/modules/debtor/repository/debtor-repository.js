...
class DebtorRepository {
    ...
    async getDebtByDebtorId(debtId, displayFieldsUsers, shouldIncludeAddress = false) {
        // Обробляємо поля - замінюємо 'address' на адресу з таблиці ower.address тільки для dobromyl
        const fields = displayFieldsUsers.map(field => {
            if (field === 'address') {
                return shouldIncludeAddress ? 'addr.address as address' : "'' as address";
            }
            return `o.${field}`;
        }).join(', ');

        let sql = `SELECT ${fields} FROM ower.ower o`;

        // Додаємо JOIN з ower.address тільки якщо потрібна адреса І це dobromyl
        if (shouldIncludeAddress && displayFieldsUsers.includes('address')) {
            sql += ` LEFT JOIN ower.address addr ON (o.name = addr.name AND o.identification = addr.ipn)`;
        }
    ...
    async insertPhoneByClientId(clientId, phoneNumber,debtor) {
        ...
        if (phoneNumber && phoneNumber.trim() !== '') {
            // Є телефон - зберігаємо з номером, hasnumber = true
            sql = `INSERT INTO ower.phone (clientid, phone, ischecked, hasnumber, name, ipn) VALUES ($1, $2, true, true, $3, $4)`;
            params = [clientId, phoneNumber.trim(), debtor.name, debtor.identification];
        } else {
            // Немає телефону - позначаємо як перевірений, hasnumber = false
            sql = `INSERT INTO ower.phone (clientid, ischecked, hasnumber, name, ipn) VALUES ($1, true, false, $2, $3)`;
    ...
    async insertPhoneByDebtor(phoneNumber,debtor) {
        ...
        if (phoneNumber && phoneNumber.trim() !== '') {
            // Є телефон - зберігаємо з номером, hasnumber = true
            sql = `INSERT INTO ower.phone (phone, ischecked, hasnumber, name, ipn) VALUES ($1, true, true, $2, $3)`;
            params = [phoneNumber.trim(), debtor.name, debtor.identification];
        } else {
            // Немає телефону - позначаємо як перевірений, hasnumber = false
            sql = `INSERT INTO ower.phone (ischecked, hasnumber, name, ipn) VALUES (true, false, $1, $2)`;
...
async findDebtByFilter(limit, offset, title, whereConditions = {}, displayFieldsUsers = [], sortBy = 'name', sortDirection = 'asc', shouldIncludeAddress = false) {
    const values = [];
    ...
    // Створюємо JSON поля (замінюємо 'address' на адресу з таблиці ower.address тільки для dobromyl)
    const jsonFields = displayFieldsUsers.map(field => {
        if (field === 'address') {
            return shouldIncludeAddress ? "'address', debtor_address" : "'address', ''";
        }
        return `'${field}', ${field}`;
    }).join(', ');

    // Визначаємо вираз для debtor_address залежно від shouldIncludeAddress
    const addressSelectExpression = shouldIncludeAddress
        ? `COALESCE(addr.address, '') as debtor_address`
        : `'' as debtor_address`;
    ...
    from (
        ...
        count(*) over () as cnt,
        p.phone as debtor_phone,
        p.ischecked,
        p.hasnumber,
        l.receipt_date,
        ${addressSelectExpression}
        from ower.ower o
        LEFT JOIN (
            SELECT DISTINCT ON (name, ipn)
                ...
                hasnumber
            FROM ower.phone
            ORDER BY name, ipn, hasnumber DESC NULLS LAST, phone NULLS LAST
        ) p ON (
            o.name = p.name AND
    ...
    // Додаємо JOIN з ower.address тільки для dobromyl
    if (shouldIncludeAddress) {
        sql += `
        LEFT JOIN ower.address addr ON (
            o.name = addr.name AND
            o.identification = addr.ipn
        )`;
    }
    ...
    if (sortBy === 'total_debt') {
        ...
        // Сортування по адресі
        sql += ` order by debtor_address ${safeSortDirection.toUpperCase()}`;
    } else {
    ...
    async getLatestRegistryDate() {
        try {
            const sql = `SELECT MAX(date) as latest_date FROM ower.ower`;
            const result = await sqlRequest(sql);

            if (!result || result.length === 0 || !result[0].latest_date) {
                Logger.info('Локальна БД порожня або не має дати реєстру');
                return null;
            }

            Logger.info('Отримано дату останнього локального реєстру', {
                latestDate: result[0].latest_date
            });

            return result[0].latest_date;
        } catch (error) {
            Logger.error('Помилка при отриманні дати останнього реєстру', {
                error: error.message,
                stack: error.stack
            });
            throw error;
    ...
    // =====================================================
    // МЕТОДИ ДЛЯ РОБОТИ З АДРЕСАМИ (таблиця ower.address)
    // =====================================================

    /**
     * Отримати адресу боржника з таблиці ower.address по name та ipn
     */
    async getAddressByDebtor(name, ipn) {
        const sql = `
            SELECT id, history_record_id, name, ipn, address, created_at, updated_at
            FROM ower.address
            WHERE name = $1 AND ipn = $2
            LIMIT 1
        `;

        try {
            const result = await sqlRequest(sql, [name, ipn]);
            return result.length > 0 ? result[0] : null;
        } catch (error) {
            Logger.error('Помилка при отриманні адреси боржника', {
                name,
                ipn,
                error: error.message
            });
    ...
    /**
     * Оновити або створити адресу боржника в таблиці ower.address
     * Використовує UPSERT (INSERT ... ON CONFLICT DO UPDATE)
     */
    async updateDebtorAddress(debtorId, address) {
        Logger.info('updateDebtorAddress: початок', { debtorId, address });

        // Спочатку отримуємо дані боржника
        const debtorSql = `SELECT id, name, identification FROM ower.ower WHERE id = $1`;
        const debtorResult = await sqlRequest(debtorSql, [debtorId]);

        Logger.info('updateDebtorAddress: результат пошуку боржника', {
            debtorId,
            found: debtorResult?.length > 0,
            debtor: debtorResult?.[0]
        });

        if (!debtorResult || debtorResult.length === 0) {
            throw new Error(`Debtor with ID ${debtorId} not found`);
        }

        const debtor = debtorResult[0];
        const name = debtor.name;
        const ipn = debtor.identification;

        // Спробуємо знайти history_record_id (порівнюємо по name + останні 3 цифри ІПН)
        let historyRecordId = null;
        try {
            const lastThreeDigits = ipn ? ipn.toString().slice(-3) : null;
            const historySql = `
                SELECT id FROM ower.ower_history
                WHERE person_name = $1
                  AND identification = $2
                ORDER BY registry_date DESC
                LIMIT 1
            `;
            const historyResult = await sqlRequest(historySql, [name, lastThreeDigits]);
            if (historyResult && historyResult.length > 0) {
                historyRecordId = historyResult[0].id;
            }
        } catch (err) {
            Logger.warn('Не вдалося знайти history_record_id', { name, error: err.message });
        }

        // UPSERT адреси
        const sql = `
            INSERT INTO ower.address (history_record_id, name, ipn, address, created_at, updated_at)
            VALUES ($1, $2, $3, $4, NOW(), NOW())
            ON CONFLICT (name, ipn) DO UPDATE
            SET
                address = EXCLUDED.address,
                history_record_id = COALESCE(EXCLUDED.history_record_id, ower.address.history_record_id),
                updated_at = NOW()
            RETURNING id, name, ipn, address
        `;

        Logger.info('updateDebtorAddress: виконання UPSERT', {
            historyRecordId,
            name,
            ipn,
            address
        ...
        try {
            const result = await sqlRequest(sql, [historyRecordId, name, ipn, address]);
            Logger.info('updateDebtorAddress: результат UPSERT', { result });
            ...
            Logger.error('Помилка при оновленні адреси боржника', {
                debtorId,
                name,
                ipn,
                address,
    ...
    /**
     * Видалити адресу боржника з таблиці ower.address
     */
    async deleteDebtorAddress(debtorId) {
        // Спочатку отримуємо дані боржника
        const debtorSql = `SELECT id, name, identification FROM ower.ower WHERE id = $1`;
        const debtorResult = await sqlRequest(debtorSql, [debtorId]);
        ...
        const sql = `
            DELETE FROM ower.address
            WHERE name = $1 AND ipn = $2
            RETURNING id, name, ipn
        `;
        ...
        try {
            ...
            if (Array.isArray(result) && result.length > 0) {
                return { deleted: true, ...result[0] };
            }

            if (result && result.rows && result.rows.length > 0) {
                return { deleted: true, ...result.rows[0] };
            }

            // Якщо нічого не видалено - це не помилка, просто не було адреси
            return { deleted: false, name, ipn, message: 'Address not found' };
        } catch (error) {
            Logger.error('Помилка при видаленні адреси боржника', {
                debtorId,
                name,
                ipn,
                error: error.message
