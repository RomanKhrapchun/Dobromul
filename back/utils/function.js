...
/**
 * Форматує ІПН до 10 символів
 * Якщо ІПН менше 10 символів - заповнює X зліва
 * Якщо ІПН більше 10 символів - обрізає до 10
 * @param {string|number} identification - ІПН (може бути частковий)
 * @returns {string} - Форматований ІПН (завжди 10 символів)
 */
const formatIPN = (identification) => {
    if (!identification) {
        return 'XXXXXXXXXX'; // Якщо немає ІПН - повертаємо 10 X
    }

    // Конвертуємо в рядок та видаляємо пробіли
    let ipnStr = String(identification).trim();

    // Видаляємо всі символи крім цифр
    const cleanedIPN = ipnStr.replace(/[^\d]/g, '');

    // Якщо після очищення нічого не залишилось - повертаємо 10 X
    if (cleanedIPN.length === 0) {
        return 'XXXXXXXXXX';
    }

    // Використовуємо очищений ІПН
    ipnStr = cleanedIPN;

    // Якщо ІПН вже 10 символів - повертаємо як є
    if (ipnStr.length === 10) {
        return ipnStr;
    }

    // Якщо ІПН більше 10 символів - обрізаємо до 10
    if (ipnStr.length > 10) {
        return ipnStr.substring(0, 10);
    }

    // Якщо ІПН менше 10 символів - заповнюємо X зліва
    const padding = 'X'.repeat(10 - ipnStr.length);
    return padding + ipnStr;
};
